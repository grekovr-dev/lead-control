...
final class EloquentFunnelTrendsReadModelTest extends TestCase
{
    ...
    public function test_it_keeps_sparse_daily_rows_and_fills_missing_metrics_with_zeroes(): void
    {
        $this->createClick('click-1', 'visitor-1', '2026-03-29 10:00:00');
        $this->createVisit('visit-1', 'visitor-2', '2026-03-30 10:05:00');
        $this->createLead('lead-1', 'visitor-3', null, '2026-03-31 10:30:00');
        ...
        $firstDay = $report->rows[0];
        $this->assertSame('2026-03-29', $firstDay->date->format('Y-m-d'));
        $this->assertSame(1, $firstDay->clicksCount);
        $this->assertSame(0, $firstDay->visitsCount);
        $this->assertSame(0, $firstDay->leadsCount);
        $this->assertSame(0.0, $firstDay->clicksToLeadsConversionRate);
        $this->assertSame(0.0, $firstDay->visitsToLeadsConversionRate);

        $secondDay = $report->rows[1];
        $this->assertSame('2026-03-30', $secondDay->date->format('Y-m-d'));
        $this->assertSame(0, $secondDay->clicksCount);
        $this->assertSame(1, $secondDay->visitsCount);
        $this->assertSame(0, $secondDay->leadsCount);
        $this->assertSame(0.0, $secondDay->clicksToLeadsConversionRate);
        $this->assertSame(0.0, $secondDay->visitsToLeadsConversionRate);

        $thirdDay = $report->rows[2];
        $this->assertSame('2026-03-31', $thirdDay->date->format('Y-m-d'));
        $this->assertSame(0, $thirdDay->clicksCount);
        $this->assertSame(0, $thirdDay->visitsCount);
        $this->assertSame(1, $thirdDay->leadsCount);
        $this->assertSame(0.0, $thirdDay->clicksToLeadsConversionRate);
        $this->assertSame(0.0, $thirdDay->visitsToLeadsConversionRate);
    }
